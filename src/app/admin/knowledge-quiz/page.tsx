...
function KnowledgeQuestionModal({
  q,
  onClose,
  onSaved,
}: {
  ...
  function setOption(i: number, field: keyof Option, val: string | boolean) {
    setOptions((prev) => {
      const next = [...prev];
  ...
  async function save() {
    ...
    if (!validOpts.some(o => o.isCorrect)) { setError("At least one correct answer required"); return; }

    setSaving(true);
    try {
      const payload = { question: question.trim(), options: validOpts, context: context.trim(), points, order, published };
      const res = q
        ? await fetch(`/api/admin/knowledge-quiz/${q._id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          })
        : await fetch("/api/admin/knowledge-quiz", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. What is the primary regulatory body for SIFs in India?"
            />
          </div>

          {/* Options with correct answer checkbox */}
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-[11px] font-mono uppercase tracking-widest text-muted">
                Options * <span className="normal-case font-normal">(min 2, mark correct answers)</span>
              </label>
            </div>

            {/* Column headers */}
            <div className="grid grid-cols-[24px_1fr_60px_28px] gap-2 mb-1.5 px-1">
              <span />
              <span className="text-[10px] font-mono uppercase tracking-widest text-muted">Option text</span>
              <span className="text-[10px] font-mono uppercase tracking-widest text-muted text-center">Correct</span>
              <span />
            </div>

            <div className="space-y-2">
              {options.map((opt, i) => (
                <div key={i} className="grid grid-cols-[24px_1fr_60px_28px] gap-2 items-center">
          ...
          {/* Points + Order + Published */}
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-[11px] font-mono uppercase tracking-widest text-muted mb-1.5">Points</label>
              <input
                type="number"
                value={points}
          ...
          <div>
            <label className="block text-[11px] font-mono uppercase tracking-widest text-muted mb-1.5">
              Explanation <span className="normal-case font-normal">(shown after user answers)</span>
            </label>
            <textarea
              value={context}
...
export default function AdminKnowledgeQuiz() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/admin/knowledge-quiz");
  ...
  async function del(q: Question) {
    ...
    await fetch(`/api/admin/knowledge-quiz/${q._id}`, { method: "DELETE" });
    load();
  }
  ...
  async function move(q: Question, dir: -1 | 1) {
    ...
    await Promise.all([
      fetch(`/api/admin/knowledge-quiz/${q._id}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ order: other.order }),
      }),
      fetch(`/api/admin/knowledge-quiz/${other._id}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" },
  ...
  const sorted = [...questions].sort((a, b) => a.order - b.order);
  const totalPoints = sorted.filter(q => q.published).reduce((sum, q) => sum + q.points, 0);
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[14px] text-muted mt-1">
            {questions.length} question{questions.length !== 1 ? "s" : ""} · {totalPoints} total points · shown on "Test your readiness" in SIF-101
          </p>
        </div>
        <button
      ...
      <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-[12px]">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-[13px] font-semibold text-blue-900 mb-1">Knowledge Quiz vs Suitability Quiz</p>
            <p className="text-[12px] text-blue-700 leading-relaxed">
              <strong>Knowledge Quiz:</strong> Tests user understanding of SIF concepts (correct/incorrect answers, points-based scoring).
              <br />
              <strong>Suitability Quiz:</strong> Assesses investor profile and risk appetite (no correct answers, value-based scoring for recommendations).
            </p>
          </div>
        </div>
      </div>
            ...
            <div key={q._id} className="flex items-start gap-3 px-5 py-4 border-b border-rule last:border-0 hover:bg-surface">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] font-mono font-semibold text-teal-700 bg-teal-50 border border-teal-200 px-1.5 py-0.5 rounded">{q.points} pts</span>
                  <p className="text-[13.5px] font-semibold text-heading">{q.question}</p>
                </div>
                {/* Options with correct marker */}
                <div className="flex flex-wrap gap-1.5 mt-1.5">
                  {q.options.map((opt, i) => (
                    <span key={i} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full ${opt.isCorrect ? 'bg-green-50 border-green-300' : 'bg-surface border-rule'} border text-[11px] ${opt.isCorrect ? 'text-green-700' : 'text-body'}`}>
                      <span className="font-mono text-muted">{String.fromCharCode(65 + i)}.</span>
                      {opt.text}
                      {opt.isCorrect && <CheckCircle2 className="size-3 text-green-600" />}
                    </span>
                  ))}
                </div>
                {q.context && (
                  <p className="text-[11.5px] text-muted mt-1.5 italic line-clamp-2">💡 {q.context}</p>
                )}
              </div>
      ...
      {modal !== undefined && (
        <KnowledgeQuestionModal q={modal} onClose={() => setModal(undefined)} onSaved={load} />
      )}
    </div>
  );
}
