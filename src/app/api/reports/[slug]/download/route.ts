...
export async function GET(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  // Special handling for May 2026 report
  if (slug === "may-2026") {
    try {
      await logClientActivity(userId, "Booklet", `Downloaded report booklet: May 2026`);
    } catch (err) {
      console.error("Booklet download activity logger error:", err);
    }

    // Serve the PDF file from public directory
    const filePath = path.join(process.cwd(), "public", "reports", "SIF-Monthly-Report-May2026.pdf");
    ...
    if (fs.existsSync(filePath)) {
      ...
      return new NextResponse(fileBuffer, {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="SIF-Monthly-Report-May2026.pdf"`,
      ...
      return NextResponse.json({ 
        error: "PDF file not found. Please ensure the file is placed at public/reports/SIF-Monthly-Report-May2026.pdf" 
      }, { status: 404 });
    }
  }
