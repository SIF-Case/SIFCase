...
import { ArrowUpRight, Plus, X, Calendar, Sparkles, Shield, Layers, Activity, Zap, Target } from "lucide-react";
import { FUNDS, type Fund } from "@/lib/data";

type Period = "1M" | "3M" | "6M" | "1Y" | "YTD" | "ALL";
type View = "Cumulative" | "Drawdown" | "Rolling 12M" | "Volatility";

const PALETTE = ["#6aa6ff", "#7ad9b4", "#f0b95a", "#e98aab", "#b48aff"];
...
// Market phases overlay (relative position 0-1 across "ALL" window)
const PHASES = [
  { start: 0.00, end: 0.18, label: "Bull Phase", color: "var(--color-positive)" },
  { start: 0.18, end: 0.34, label: "Correction", color: "var(--color-negative)" },
  { start: 0.34, end: 0.58, label: "Recovery", color: "var(--color-gold)" },
  { start: 0.58, end: 0.82, label: "Bull Phase", color: "var(--color-positive)" },
  { start: 0.82, end: 1.00, label: "Volatile", color: "var(--color-primary)" },
];

const EVENTS = [
  { at: 0.08, label: "SEBI SIF Framework" },
  { at: 0.26, label: "Union Budget 2024" },
  { at: 0.42, label: "Fed Rate Cut" },
  { at: 0.60, label: "Market Correction" },
  { at: 0.88, label: "General Elections" },
];

const PRESETS: { id: string; label: string; icon: typeof Sparkles; pick: () => string[] }[] = [
  ...
  { id: "ls", label: "Long-Short Funds", icon: Activity, pick: () => FUNDS.filter((f) => f.strategy === "Long-Short").slice(0, 3).map((f) => f.id) },
...
function toCumulative(s: number[]): number[] {
  const base = s[0];
  return s.map((v) => ((v - base) / base) * 100);
}
function toDrawdown(s: number[]): number[] {
  let peak = s[0];
  return s.map((v) => {
...
function toRolling(s: number[]): number[] {
  const w = Math.min(12, Math.max(2, Math.floor(s.length / 3)));
  return s.map((_, i) => {
    if (i < w) return 0;
    return ((s[i] - s[i - w]) / s[i - w]) * 100;
  });
}
function toVolatility(s: number[]): number[] {
  const w = Math.min(6, Math.max(2, Math.floor(s.length / 4)));
  const rets = s.map((v, i) => (i === 0 ? 0 : (v - s[i - 1]) / s[i - 1]));
  return rets.map((_, i) => {
    if (i < w) return 0;
...
function benchmark(n: number, seed = 1): number[] {
  const out: number[] = [];
  let v = 100;
  for (let i = 0; i < n; i++) {
    v += 0.10 + Math.sin(i * 0.6 + seed) * 0.32;
    out.push(+v.toFixed(2));
  }
  return out;
}
function benchSpark(id: string) {
  if (id === "_bench_bse500") return benchmark(36, 1);
  if (id === "_bench_nifty50") return benchmark(36, 2);
  return benchmark(36, 3);
}

export function BuildYourCompare() {
  const defaults = useMemo(
    () => [...FUNDS].sort((a, b) => b.returns.y1 - a.returns.y1).slice(0, 3).map((f) => f.id),
    [],
  );
  ...
  const selBench = benches.map((id) => BENCHMARKS.find((b) => b.id === id)!).filter(Boolean);
  ...
  const series = useMemo(() => {
    ...
    const arr = selected.map((f, i) => ({
      id: f.id,
      name: f.name.replace(/\s*Fund\s*$/, ""),
      color: PALETTE[i % PALETTE.length],
      data: transform(f.spark),
      isBench: false,
    }));
    selBench.forEach((b) =>
      arr.push({ id: b.id, name: b.name, color: b.color, data: transform(benchSpark(b.id)), isBench: true }),
    );
    return arr;
  ...
  const addableBench = BENCHMARKS.filter((b) => !benches.includes(b.id));
  ...
  return (
    <div className="bg-surface border border-border rounded-xl overflow-hidden flex flex-col">
      <div className="px-5 py-4 border-b border-border flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        <div>
          <div className="text-[11px] font-mono uppercase tracking-widest text-primary">Build Your Compare</div>
          <h3 className="mt-1 text-[15px] font-semibold">Live multi-fund performance lab</h3>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="h-7 px-2 inline-flex items-center gap-1 rounded-full border border-border-strong bg-surface text-[11px] text-muted-foreground hover:text-foreground"
          >
      ...
      <div className="px-5 py-3 border-b border-border flex flex-wrap items-center gap-2">
        {selected.map((f, i) => (
          <span key={f.id} className="inline-flex items-center gap-2 pl-2 pr-1 py-1 rounded-full bg-surface-2 border border-border text-[11px]">
            ...
            <button onClick={() => setPicked(picked.filter((id) => id !== f.id))} className="p-0.5 rounded hover:bg-border-strong">
              <X className="size-3" />
            </button>
          </span>
        ))}
        {selBench.map((b) => (
          <span key={b.id} className="inline-flex items-center gap-2 pl-2 pr-1 py-1 rounded-full bg-surface-2 border border-dashed border-border text-[11px] text-muted-foreground">
            ...
            <button onClick={() => setBenches(benches.filter((id) => id !== b.id))} className="p-0.5 rounded hover:bg-border-strong">
              <X className="size-3" />
            </button>
          </span>
        ))}
        <div className="relative">
          <button
            ...
            className="h-7 px-3 inline-flex items-center gap-1 rounded-full border border-dashed border-border-strong bg-surface text-[11px] text-muted-foreground hover:text-foreground"
          >
          ...
          {addOpen && (
            <div className="absolute z-30 top-full mt-2 left-0 w-72 bg-surface border border-border-strong rounded-xl shadow-2xl shadow-black/50 p-2 max-h-[280px] overflow-auto">
              ...
              {addableFunds.map((f) => (
                <button
                  key={f.id}
                  onClick={() => { if (picked.length < 5) setPicked([...picked, f.id]); setAddOpen(false); }}
                  className="w-full text-left px-2 py-1.5 rounded hover:bg-surface-2 text-[12px] truncate"
                >
                  {f.name}
                </button>
              ))}
              ...
              {addableBench.map((b) => (
                <button
                  key={b.id}
                  onClick={() => { setBenches([...benches, b.id]); setAddOpen(false); }}
                  className="w-full text-left px-2 py-1.5 rounded hover:bg-surface-2 text-[12px] truncate"
                >
                  {b.name}
                </button>
      ...
      {/* Market phases ribbon */}
      <div className="px-5 pt-3">
        <div className="relative h-5 rounded-md overflow-hidden border border-border flex">
          {PHASES.map((p, i) => (
            <div
              key={i}
              className="h-full flex items-center justify-center text-[9px] font-mono uppercase tracking-widest"
              style={{
                width: `${(p.end - p.start) * 100}%`,
                background: `color-mix(in oklab, ${p.color} 12%, transparent)`,
                color: `color-mix(in oklab, ${p.color} 80%, var(--color-foreground))`,
              }}
            >
              {p.label}
            </div>
      ...
      <div className="px-2 py-3 flex-1 relative">
        ...
        >
          ...
          {PHASES.map((p, i) => (
            <rect
              key={`phase-${i}`}
              x={PADL + p.start * innerW}
              y={PADT}
              width={(p.end - p.start) * innerW}
              height={innerH}
              fill={`color-mix(in oklab, ${p.color} 5%, transparent)`}
            />
          ))}
          ...
          {view !== "Volatility" && (
            <line x1={PADL} x2={W - PADR} y1={zeroY} y2={zeroY} stroke="var(--color-border-strong)" />
          )}
        ...
        {hoverI !== null && (
          <div
            className="pointer-events-none absolute top-3 bg-surface border border-border-strong rounded-lg shadow-2xl shadow-black/50 p-2.5 text-[11px] min-w-[180px]"
              ...
              {series.map((s) => (
                <div key={s.id} className="flex items-center gap-2">
                  ...
                  <span className="tabular font-mono">
                    {s.data[hoverI] >= 0 ? "+" : ""}{s.data[hoverI].toFixed(2)}{unit === "%" ? "%" : "%a"}
                  </span>
                </div>
              ))}
            </div>
      ...
      <div className="px-5 pb-3 -mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] font-mono uppercase tracking-widest">
        <span className="text-muted-foreground">Key events:</span>
        {EVENTS.map((e, i) => (
          <span key={i} className="inline-flex items-center gap-1 text-foreground/80">
            <span className="size-1.5 rounded-full" style={{ background: "var(--color-gold)" }} /> {e.label}
          </span>
        ))}
      </div>
      ...
      <div className="px-5 py-4 border-t border-border bg-surface-2/40">
        <div className="flex items-center justify-between mb-2.5">
          ...
          <Link to="/analytics" className="text-[11px] font-mono uppercase tracking-widest text-primary hover:underline inline-flex items-center gap-1">
            Launch full analytics terminal <ArrowUpRight className="size-3" />
          </Link>
        </div>
          ...
          {PRESETS.map((p) => {
            ...
            return (
              <button
                ...
                className="inline-flex items-center gap-1.5 h-7 px-3 rounded-full border border-border bg-surface text-[11px] hover:bg-surface-2 hover:border-border-strong transition"
              >
    ...
    <div className="inline-flex p-1 rounded-full border border-border-strong bg-surface">
      {options.map((o) => (
        <button
          key={o}
          onClick={() => onChange(o)}
          className={`h-7 px-2.5 rounded-full text-[11px] font-medium transition whitespace-nowrap ${
            active === o ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
          }`}
        >
          {o}
        </button>
